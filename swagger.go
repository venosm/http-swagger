...
type Config struct {
	// The url pointing to API definition (normally doc.json, swagger.json, swagger.yaml, openapi.json or openapi.yaml). Default is `doc.json`.
	URL                      string
	DocExpansion             string
	DomID                    string
	InstanceName             string
...
// TryItOutEnabled enables/disables the "Try it out" feature in the Swagger UI.
// This is particularly useful for OpenAPI 3.0 specifications.
func TryItOutEnabled(tryItOutEnabled bool) func(*Config) {
	return func(c *Config) {
		c.TryItOutEnabled = tryItOutEnabled
	}
}

// RequestInterceptor allows customization of request processing for OpenAPI 3.0.
func RequestInterceptor(interceptor string) func(*Config) {
	return func(c *Config) {
		c.RequestInterceptor = template.JS(interceptor)
	}
}

// ResponseInterceptor allows customization of response processing for OpenAPI 3.0.
func ResponseInterceptor(interceptor string) func(*Config) {
	return func(c *Config) {
		c.ResponseInterceptor = template.JS(interceptor)
	}
}
