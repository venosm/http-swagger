...
type mockedOpenAPISwag struct{}

func (s *mockedOpenAPISwag) ReadDoc() string {
	return `{
    "openapi": "3.0.3",
    "info": {
        "description": "This is a sample OpenAPI 3.0 server.",
...
func TestWrapHandler(t *testing.T) {
	...
	for _, test := range tests {
		...
		wOpenAPI := performRequest(http.MethodGet, test.RootFolder+"openapi.json", router)
		assert.Equal(t, http.StatusOK, wOpenAPI.Code)
		assert.Equal(t, "application/json; charset=utf-8", wOpenAPI.Header().Get("content-type"))

		wOpenAPIYaml := performRequest(http.MethodGet, test.RootFolder+"openapi.yaml", router)
		assert.Equal(t, http.StatusOK, wOpenAPIYaml.Code)
		assert.Equal(t, "application/x-yaml; charset=utf-8", wOpenAPIYaml.Header().Get("content-type"))
...
func TestTryItOutEnabled(t *testing.T) {
	...
	cfg = newConfig()
	assert.True(t, cfg.TryItOutEnabled) // Default should be true

	cfg = newConfig(TryItOutEnabled(false))
	assert.False(t, cfg.TryItOutEnabled)

	cfg = newConfig(TryItOutEnabled(true))
	assert.True(t, cfg.TryItOutEnabled)
}

func TestRequestInterceptor(t *testing.T) {
	var cfg *Config

	expected := "(req) => { console.log(req); return req; }"
	cfg = newConfig(RequestInterceptor(expected))
	assert.Equal(t, template.JS(expected), cfg.RequestInterceptor)
}

func TestResponseInterceptor(t *testing.T) {
	var cfg *Config

	expected := "(res) => { console.log(res); return res; }"
	cfg = newConfig(ResponseInterceptor(expected))
	assert.Equal(t, template.JS(expected), cfg.ResponseInterceptor)
}
